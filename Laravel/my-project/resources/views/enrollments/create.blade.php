            ...
            .then(data => {
                ...
                if (Object.keys(data).length === 0) {
                    ...
                    Object.keys(data).forEach(year => {
                        ...
                        const subjectList = document.createElement('div');
                        subjectList.className = 'grid grid-cols-1 gap-2'; // Makes subjects align in two columns
...
data[year].forEach(subject => {
    const label = document.createElement('label');
    label.className = 'flex items-center justify-between'; // Aligns text left, checkbox right
